...
namespace Haytham
{
    public class Eye
    {
        ...
        //private array x,y
        public List<int> LockedPupil_XList = new List<int>();
        public List<int> LockedPupil_YList = new List<int>();
        public List<int> LockedPupil_XDeferenceList = new List<int>();
        public List<int> LockedPupil_YDeferenceList = new List<int>();

        //........................

        public EyeData[] eyeData = new EyeData[500];

        public bool LastClientEnable = false;
        ...
        public void blink(ref bool Blink, ref bool DoubleBlink)
        {
            ...
            if (DoubleBlink) METState.Current.METCoreObject.SendToForm("DbBlink0", "BlinkIndicator");
            if (Blink) METState.Current.METCoreObject.SendToForm("Blink0", "BlinkIndicator");
                   ...
                   if (WaitForDoubleBlink)
                    {
                        if (doubleblinkWaitingTime_Counter < doubleblinkWaitingTime)
                        {
                            ...
                            DoubleBlink = true;
                            METState.Current.METCoreObject.SendToForm("DbBlink1", "BlinkIndicator");
                        }
                        else
                        {
                            WaitForDoubleBlink = false;
                            ...
                            Blink = true;
                            METState.Current.METCoreObject.SendToForm("Blink1", "BlinkIndicator");
                        }
                    }
                    else
                    {
            ...
            else
            {
             ...
             CloseEye = true;
                ...
                #region RecipeDemo
                bool VolumeIsOn = METState.Current.server.GetCondition(METState.Current.server.activeScreen, "RecipeDemo") & METState.Current.server.GetCondition(METState.Current.server.activeScreen, "Volume");

                    if (VolumeIsOn)
                    {
                        METState.Current.server.SendToClient("RecipeDemo", "Monitor", false, VolumeIsOn);
                        METState.Current.server.SendToClient("VolumeZone", "Monitor", false, VolumeIsOn);
                        METState.Current.server.SendToClient("Off", "Monitor", false, VolumeIsOn);
                    }
                #endregion RecipeDemo
   ...
   (int)(METState.Current.MaxPupilScale * METState.Current.IrisDiameter),
    0.55, 4);//????????????????????????????????????????????????    ,2)
        ...
        private bool CheckROIRegion(Point ROIc, Size ROIsize, Size imgSize)
        {
            bool OK = true;
